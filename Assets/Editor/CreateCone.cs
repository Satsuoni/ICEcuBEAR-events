...
public class CreateCone : ScriptableWizard
{
    ...
    void OnWizardCreate()
    {
        ...
        else
        {
            ...
            mesh.name = meshName;
            int multiplier = (outside ? 1 : 0) + (inside ? 1 : 0);
            int offset = (outside && inside ? numVertices : 0);
            Vector3[] vertices = new Vector3[multiplier * numVertices+2]; // 0..n-1: top, n..2n-1: bottom
            Vector3[] normals = new Vector3[multiplier  * numVertices+2];
            Vector2[] uvs = new Vector2[ multiplier * numVertices+2];
            int[] tris;
            float slope = Mathf.Atan((radiusBottom - radiusTop) / length); // (rad difference)/height
            float slopeSin = Mathf.Sin(slope);
            float slopeCos = Mathf.Cos(slope);
            ...
            for (i = 0; i < numVertices; i++)
            {
                ...
                vertices[i + 2] = new Vector3(radiusBottom * angleCos, radiusBottom * angleSin, length);

            
                    normals[0] = new Vector3(angleHalfCos * slopeCos, angleHalfSin * slopeCos, -slopeSin);
                normals[1] = -new Vector3(angleHalfCos * slopeCos, angleHalfSin * slopeCos, -slopeSin);

                if (radiusBottom == 0)
                    normals[i + 2] = new Vector3(angleHalfCos * slopeCos, angleHalfSin * slopeCos, -slopeSin);
                else
                    normals[i + 2] = new Vector3(angleCos * slopeCos, angleSin * slopeCos, -slopeSin);

                uvs[0] = new Vector2(0, 1);
                uvs[1] = new Vector2(0, 1);
                uvs[i + 2] = new Vector2(1.0f * i / numVertices, 0);

                if (outside && inside)
                {
                    // vertices and uvs are identical on inside and outside, so just copy
             
                    vertices[i + 2+ numVertices] = vertices[i + 2];
                   
                    uvs[i + 2+  numVertices] = uvs[i + 2];
                }
                if (inside)
                {
                    // invert normals
                
                    normals[i + 2 + offset] = -normals[i + 2];
                }
            }
            mesh.vertices = vertices;
            mesh.normals = normals;
