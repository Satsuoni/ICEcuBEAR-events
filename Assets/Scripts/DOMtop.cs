...
*/
public class DOMTop : MonoBehaviour { //DOMTop is simply the name used in unity and does not refer to IceCube DOM
    ...
    public Dictionary<int, dStringTop> strings = new Dictionary<int, dStringTop>();
	...
	{
		foreach(KeyValuePair<int, dStringTop> iString in strings)
		{
    ...
    private void Start() {
        ...
        var fileStream = new FileStream(@"Assets/geo_inice_icetop.csv", FileMode.Open, FileAccess.Read);
        using (var streamReader = new System.IO.StreamReader(fileStream, System.Text.Encoding.UTF8))
        {
            ...
            while ((line = streamReader.ReadLine()) != null)
            {
                ...
                Debug.Log(fields[1]);
                Debug.Log(fields[2]);
                int stringID = int.Parse(fields[1]);
                int domID = int.Parse(fields[2]);
                float x = float.Parse(fields[3]);
                float y = float.Parse(fields[4]);
                float z = float.Parse(fields[5]) + 550f; //move icecube coord system by 550 m
                ...
                if(!strings.ContainsKey(stringID))
                {
                    dStringTop theString = new GameObject("String " + stringID).AddComponent<dStringTop>().Initialize(this, stringID, pos);
                    strings.Add(stringID, theString);
                }
...
public class dStringTop : MonoBehaviour
{
    private static readonly Config cfg = new Config();
    public int id;
    private Mesh mesh;
    ...
    private void Start() {
        ...
        gameObject.AddComponent<SingleString>();
    }
    
    public dStringTop Initialize (DOMTop parent, int id, Vector3 pos) {
        this.id = id;
    ...
    public GameObject AddDOM(DOMTop parent, int domID, Vector3 pos)
    {      
		if(prefab==null)
		{
