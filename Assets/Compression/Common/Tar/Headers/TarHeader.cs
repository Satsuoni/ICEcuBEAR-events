...
namespace SharpCompress.Common.Tar.Headers
{
    internal sealed class TarHeader
    {
        ...
        internal void Write(Stream output)
        {
            ...
            else
            {
                ...
                if (Size >= 0x1FFFFFFFF)
                {
                    ...
                    BinaryPrimitives.WriteInt64BigEndian(bytes12,(4), Size);
                    bytes12[0] |= 0x80;
        ...
        private long ReadSize(byte[] buffer)
        {
            ...
            {
                return BinaryPrimitives.ReadInt64BigEndian(buffer,(0x80));
            }
        ...
        private static void WriteStringBytes(byte[] name, byte[] buffer, int length)
        {
            name.CopyTo(buffer,0);
            int i = Math.Min(length, name.Length);
            for (int k = i; k < buffer.Length; k++) buffer[i] = 0;
