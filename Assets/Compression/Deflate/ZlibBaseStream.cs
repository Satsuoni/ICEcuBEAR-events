...
namespace SharpCompress.Compressors.Deflate
{
    ...
    internal class ZlibBaseStream : Stream
    {
        ...
        {
            get => _workingBuffer = _workingBuffer!=null?_workingBuffer: new byte[_bufferSize];
        }
            ...
            if (_streamMode == StreamMode.Writer)
            {
                ...
                if (_flavor == ZlibStreamFlavor.GZIP)
                {
                    if (_wantCompress)
                    {
                        ...
                        BinaryPrimitives.WriteInt32LittleEndian(intBuf,0, crc.Crc32Result);

                        _stream.Write(intBuf,0,4);
                        int c2 = (int)(crc.TotalBytesRead & 0x00000000FFFFFFFF);
                        BinaryPrimitives.WriteInt32LittleEndian(intBuf,0, c2);
            ...
            else if (_streamMode == StreamMode.Reader)
            {
                if (_flavor == ZlibStreamFlavor.GZIP)
                {
                    if (!_wantCompress)
                    {
                        ...
                        Int32 crc32_expected = BinaryPrimitives.ReadInt32LittleEndian(trailer,0);
                        Int32 crc32_actual = crc.Crc32Result;
                        Int32 isize_expected = BinaryPrimitives.ReadInt32LittleEndian(trailer,4);
